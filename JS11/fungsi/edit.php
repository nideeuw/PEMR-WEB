...
if (!empty($_SESSION['username'])) {
    ...
    if (!empty($_GET['jabatan'])) {
        $id = antiInjection(koneksi: $koneksi, data: $_POST['id']);
        $jabatan = antiInjection(koneksi: $koneksi, data: $_POST['jabatan']);
        $keterangan = antiInjection(koneksi: $koneksi, data: $_POST['keterangan']);

        $query = "UPDATE jabatan SET jabatan = '$jabatan', keterangan = '$keterangan' WHERE id = '$id'";

        if (pg_query($koneksi, $query)) {
            pesan(key: 'success', pesan: "Jabatan Telah Diubah.");
        } else {
            pesan(key: 'danger', pesan: "Mengubah Jabatan Karena: " . pg_last_error($koneksi));
        }
